 ...
 *
 * Primary path is the async Clipboard API; on failure (API missing or
 * permission denied, e.g. inside a hardened Electron shell) it falls back
 * to the legacy execCommand trick so the copy button never fails silently.
 *
...
export const copyToClipboard = async (text: string): Promise<boolean> => {
	try {
		...
		// Clipboard API 不可用或权限被拒，走下方 execCommand 兜底
	}
	try {
		const textarea = document.createElement('textarea');
		textarea.value = text;
		// readonly 防止移动端弹键盘；fixed + opacity 让它不引起布局抖动/滚动
...
export function formatNumber(num: number): string {
	// 容错：调用方拿到的可能是 undefined（后端少了字段、数据还在路上）。
	// 不设这一层的话，`undefined.toLocaleString()` 会直接把整个渲染树打崩，
	// 而这本来只是"一个角标没数字"而已。
	const n = Number(num);
