...
export interface CreateSessionRequest {
	...
	knowledge_config?: SessionKnowledgeConfig | null;
	/** Optional initial working directory. null = workspace root. */
	cwd?: string | null;
...
export interface InterruptSessionResponse {
	session_id: string;
	/** true = cancel was broadcast to a running reply; false = target was already idle (no ReplyEndEvent will arrive). */
	interrupted: boolean;
}
...
export interface ChatRequest {
	...
	/**
	 * Implicit context blocks (workspace cwd / git, recent history, available
	 * tools) that the model should see but the user should not. Sent as
	 * dedicated blocks alongside ``input`` so the backend can pre-prepend the
	 * derived text into the LLM message (``internal``) without polluting the
	 * user-visible history (``display``).
	 */
	auto_context?: ContentBlock[];
	/**
	 * Skill ids the user attached via the slash command menu in the input.
	 * Bridge.js will rebuild the skill spec text and prepend it to the LLM
	 * message in the ``internal`` channel; the user-visible bubble renders
	 * the chip row from message.metadata.selected_skill_ids (frontend only).
	 *
	 * Required only when the host wants the bridge to compose the spec
	 * payload — if ``auto_context`` already carries the skill text, this can
	 * be omitted. Both routes are kept for flexibility.
	 */
...
export interface HubInfo {
	...
	/**
	 * The hub publishes a category taxonomy we can filter by. Every current
	 * skill hub does; the flag exists so a future source can opt out rather
	 * than us hardcoding which is which.
	 */
	supports_categories?: boolean;
	/**
	 * The hub can pick a skill from a free-text task description. Only
	 * OpenAgentSkill does — gates the "match by task" affordance.
	 */
	supports_resolve?: boolean;
}

/** One entry of a hub's category taxonomy. */
export interface HubCategory {
	id: string;
	/**
	 * How many skills carry this category. `null` when the hub does not say —
	 * which is not the same as zero, so don't render it.
	 */
	count: number | null;
	/** `true` when the count was aggregated from one catalog page rather than counted upstream. */
	approximate: boolean;
}

/** A skill the hub resolved from a natural-language task. */
export interface ResolvedSkill {
	...
	/** 0-100 relevance. `null` when the hub omits it. */
	match_score: number | null;
	url: string | null;
}

/** Result of asking a hub "which skill should I use for this task?". */
export interface SkillResolveResult {
	task: string;
	agent: string;
	/** The command the hub says to run — informational; we record, never execute. */
	install_command: string | null;
	selected: ResolvedSkill | null;
	alternatives: ResolvedSkill[];
	total_searched: number | null;
	/** e.g. `human_review_required`; `null` when the hub has no policy opinion. */
	policy: string | null;
...
export interface HubBrowseParams {
	...
	q?: string;
	/**
	 * Category id from `hubApi.skill.listCategories`. Filtering happens
	 * server-side upstream, so it narrows the whole catalog, not just the
...
export interface SkillCard extends HubCardBase {
	/** The hub's Chinese blurb when it has one — `description` is often English. */
...
export interface SkillView {
	...
	description: string;
	/**
	 * Snapshotted Chinese blurb. Optional because records installed before this
	 * field existed are still on disk without it.
	 */
	description_zh?: string | null;
	tags: string[];
