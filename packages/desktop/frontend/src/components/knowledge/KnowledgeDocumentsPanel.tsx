...
function RowView({ row, onCancel, onDismiss, onDelete, onOpen }: RowViewProps) {
	...
	return (
		<div
			className={cn(
				'border-border bg-card flex flex-col gap-y-2 rounded-lg border p-3 transition-all duration-200',
				openable && 'hover:-translate-y-0.5 hover:border-border hover:bg-accent hover:shadow-md cursor-pointer',
			)}
			onClick={
		...
		>
			...
			{showProgress && (
				<div className="bg-muted relative h-1 w-full overflow-hidden rounded-[2px]">
					<div
						className={cn(
