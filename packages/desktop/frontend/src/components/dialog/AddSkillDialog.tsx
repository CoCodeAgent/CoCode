import { CircleAlert, FolderUp, HardDriveDownload, PlusCircle } from 'lucide-react';
...
interface Props {
	...
	/**
	 * Imports a skill from a local folder via the Electron native folder
	 * dialog (path-based, no file-by-file upload). Resolves when the skill
	 * is in the library (and, when a session is open, the workspace).
	 */
 ...
 */
export function AddSkillDialog({ children, present, onUpload, onAddFromLibrary, onImportLocal }: Props) {
	...
	const handleImportLocal = async () => {
		...
		try {
			await onImportLocal();
			await refetch(); // 新导入的技能要立刻出现在「已安装」列表里
			close();
		} catch (e) {
			setError((e as Error).message);
		} finally {
							...
							{files.length === 0 ? (
								<Empty className="border-none py-10">
									...
									<div className="flex flex-col items-center gap-2">
										...
										{hasNativeDialog && (
											<Button variant="outline" disabled={busy} onClick={() => void handleImportLocal()}>
												<HardDriveDownload />
