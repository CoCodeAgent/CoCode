					...
					if (idx > 0) {
						...
						children.push(
							<Decoration key={`gap-${hunk.oldStart}-${hunk.newStart}`}>
								<div className="flex items-center gap-2 border-y border-dashed border-border bg-muted px-3 py-1 text-[10px] text-muted-foreground select-none">
				...
				if (hiddenLines > 0) {
					children.push(
						<Decoration key="collapsed-diff-lines">
							<button
								type="button"
								className="flex w-full items-center justify-center gap-1 border-t border-border bg-muted px-3 py-2 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
