...
function ChannelCard({
	...
	return (
		<Item
			variant="outline"
			onClick={onOpen}
			className="cursor-pointer items-start shadow-panel transition hover:border-ring"
		>
...
function ChannelTypeCard({ type, onPick }: { type: ChannelTypeSchema; onPick: () => void }) {
	return (
		<button
			onClick={onPick}
			className="group flex items-start gap-3 rounded-xl border bg-card p-4 text-left shadow-panel transition hover:border-ring"
		>
			...
			<ResizablePanelGroup orientation="horizontal">
				<ResizablePanel minSize="24rem">
					<main className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-[22px] bg-card shadow-panel">
						<div className="app-drag px-6 pt-5 pb-4">
