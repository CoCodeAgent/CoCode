			...
			<ItemContent>
				...
				<ItemTitle>
					...
					{card.auth === 'inputs' && (
						...
						<Badge className="rounded-rect-sm border-0 bg-amber-50 px-2 py-0.5 text-[10.5px] font-normal text-amber-700">
							{t('mcp.needsConfig')}
						</Badge>
					)}
			...
			<ItemActions className="items-center gap-3 self-center">
				...
				<div className="flex h-8 items-center gap-2">
					...
					{installed ? (
						...
						<Button
							className="h-7 rounded-rect px-3.5 text-xs font-normal hover:bg-accent"
...
function HubPanel({ hubId, hub, installedNames, onInstalled }: HubPanelProps) {
	...
	return (
		...
		>
			<div className="app-no-drag flex flex-col gap-y-4">
				{loading ? (
