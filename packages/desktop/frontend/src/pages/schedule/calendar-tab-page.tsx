		...
		>
			<div className="flex items-start justify-between mb-1">
				<div className="w-6 h-6 flex items-center justify-center">
					<div
						className={`text-sm ${
							isToday
								? 'bg-primary text-primary-foreground rounded-rect-sm w-6 h-6 flex items-center justify-center'
								: ''
						}`}
					>
						{day}
			...
			<div className="space-y-1">
				{events.slice(0, maxVisibleEvents).map((event) => (
					<div
						key={event.id}
						className="flex flex-row justify-between text-xs py-0.5 px-1 text-secondary-foreground rounded-sm cursor-pointer hover:bg-primary-soft"
			...
			<div className="flex-1 flex flex-col">
				...
				>
					...
					{prevMonthDays.map((day, index) => (
						<div
							key={`prev-${index}`}
							className="border-r border-b border-border! p-2 text-muted-foreground"
						>
					...
					{nextMonthDays.map((day, index) => (
						<div
							key={`next-${index}`}
							className="border-border! border-r border-b p-2 text-muted-foreground"
						>
							<div className="text-sm">{day}</div>
						</div>
					))}
