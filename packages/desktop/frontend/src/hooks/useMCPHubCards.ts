 ...
 * page it came from actually added something new, so a hub that repeats a
 * page cannot stack the same rows over and over.
 *
	...
	const load = useCallback(
		async (append: boolean, fromCursor: string | null) => {
			...
			try {
				...
				// See useSkillHubCards: filter against what is on screen first,
				// then only keep the cursor if the list actually moved forward.
